...
Let's create a bind shell. In this case, the attacker can use a command like the one below on the lab machine.
...
Terminal on the Lab Machine (Bind Shell Setup)
...
Now that the lab machine is waiting for incoming connections, we can use Netcat again with the following command to connect.
